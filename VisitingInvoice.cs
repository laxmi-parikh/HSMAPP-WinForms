...
namespace HSMAPP
{
    public partial class VisitingInvoice : Form
    {
        ...
        int Id;
        int mon = Convert.ToInt32(DateTime.Today.Month);
               ...
               Id = Convert.ToInt32(comboxdoc.SelectedValue);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.Read() != false)
                    {
                        ...
                        lbldrname.Text = Convert.ToString(reader["Name"]);
        ...
        void datafindinvoice()
        {
            try
            {
                using (con = new SqlConnection(dbconnectionstring))
                {
                    ...
                    // dataGridView2.Columns["Sr.No"].Width = 20;
                    this.dataGridView2.Columns["Id"].Visible = false;
                    this.dataGridView2.Columns["TotalAmount"].Visible = false;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                lblbillno.Text = cboxbillno.SelectedValue.ToString();
                getdoctordetails();
                datafindinvoice();
            }
            catch (Exception ex)
        ...
        public void paneltobmp()
        {
            ...
            panel1.DrawToBitmap(bm, new System.Drawing.Rectangle(0, 0, 800, 1100));
            PrintArea(panel1);
            outputFileName = @"../../Imgs/" + comName + "_Invoices" + cboxbillno.SelectedValue + ".bmp";
        ...
        private void dataGridView2_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (var gridPen = new Pen(grid.GridColor, 2))
            {
                // Draw the outer border of the DataGridView
                e.Graphics.DrawRectangle(gridPen, gridBounds.Left, gridBounds.Top, gridBounds.Width - 1, gridBounds.Height - 1);
            }
        }
        void GetBillNo()
        {
        ...
        void getinvoicedate()
        {
            try
            {
                using (con = new SqlConnection(dbconnectionstring))
                {
                    cmd = new SqlCommand("usp_GetVisitingmstBycompanyId", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@CompanyId", Convert.ToInt32(comboxcompany.SelectedValue));
                    cmd.Parameters.AddWithValue("@Month", Convert.ToInt32(combomonth.SelectedItem));
                    cmd.Parameters.AddWithValue("@year", Convert.ToInt32(comboyear.SelectedItem));
